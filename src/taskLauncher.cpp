...
int TaskLauncher::runTasks(long expeDuration)
{
   ...
   for (auto& taskInfo : tasksSet)
   {
      ...
      if (pid == 0) // proc fils
      {
         ...
         //rt_printf("[%llu][ %s ] - Semaphor %s joined.\n", rt_timer_read(), currentTaskDescriptor.fP.name, SEM_NAME);
...
int TaskLauncher::runAgent(long expeDuration)
{
   ...
   rt_printf("[%llu][ %s ] - Semaphor %s created.\n", rt_timer_read(), currentTaskDescriptor.fP.name, SEM_NAME); //cout << "["<< currentTaskDescriptor.fP.name << "]"<< "Semaphor Created." << endl;
   rt_task_wait_period(0);
   rt_alarm_start(&_endAlarm, _SEC(expeDuration), TM_INFINITE);
   ...
   { // waiting every alarm is set.
      ...
      rt_printf("[%llu][ %s ] - Semaphor %s catched !\n", rt_timer_read(), currentTaskDescriptor.fP.name, SEM_NAME); //cout << "["<< currentTaskDescriptor.fP.name << "]"<< "Semaphor catched !" << endl;
